...
def gauge_base() -> Gauge:
    ...
    c = (
        ...
        .add("", [("",HDD_text )], max_=100,  detail_label_opts = opts.GaugeDetailOpts( offset_center = [0, "-40%"]), radius="50%", title_label_opts=opts.LabelOpts(font_size=20, color="white", font_family="Microsoft YaHei"), axisline_opts=opts.AxisLineOpts(linestyle_opts=opts.LineStyleOpts(color=[(0.3, "#67e0e3"), (0.7, "#37a2da"), (1, "#fd666d")], width=10)))
    ...
    make_snapshot(snapshot, c.render(), "./cache_pic/HDD.png")

    img = Image.open('./cache_pic/HDD.png')
    area = (600, 200, 1200, 700)#width在前，height在后
    cropped_img = img.crop(area)
    #cropped_img.show()
    cropped_img.save('./cache_pic/HDD.png')
    #print(CPU)
